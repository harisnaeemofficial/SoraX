...
const sliderStyles = tv({
  slots: {
    ...
    thumb:
      'focus-visible:ring-ring block size-5 rounded-full border-2 bg-background ring-offset-background transition-colors hover:cursor-grab focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50',
  },
  variants: {
    color: {
      default: {
        ...
        {thumbsArray?.map((value, index) => (
          <SliderPrimitive.Thumb key={index} className={thumb()}>
            {showValueOnHover ? (
              ...
              >
                <div className="size-5" {...(index === 0 ? firstThumbProps : secondThumbProps)} />
              </Badge>
            ) : null}
